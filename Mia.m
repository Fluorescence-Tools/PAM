...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    h.Mia_NB_Axes(5)= axes(...
        ...
        colorbar(h.Mia_NB_Axes(3));
        
        h.Plots.NB(5)=image(zeros(1,1,3),...
        ...
        for i=1:3;
            for j=1:3                
                h.Mia_NB_Hist(i,j) = uicontrol(...
                    ...
                    'ForegroundColor', Look.Fore,...
                    'Callback',{@Update_Plots,3,1:3},...
                    'Position',[0.06+0.14*i 0.66-0.08*j, 0.12 0.06],...
                    'String','1');
            end
        end
    ...
    for i=channel
        if ~isempty(MIAData.Cor{i,1})
            ...
            if Frame==0
                ...
                Image=mean(double(MIAData.Cor{i,1}(X(1):X(2),Y(1):Y(2),Frames)),3);
            else
            ...
            switch h.Mia_Cor_Fit_Type.Value
                ...
                case 2 %%% Plots fit residual surf plot
                    ...
                    if Frame==0
                        SEM=std(double(MIAData.Cor{i,1}(X(1):X(2),Y(1):Y(2),Frames)),0,3)/sqrt(numel(Frames));
                    else
...
if any(mode==3)
    ...
    i=1;
    h.Plots.NB(1).CData=MIAData.NB.Int{i};
    ...
    h.Plots.NB(3).CData=MIAData.NB.Eps{i};
    ...
    Use=MIAData.NB.Int{i}>=str2double(h.Mia_NB_Hist(1,1).String) &...
        MIAData.NB.Int{i}<=str2double(h.Mia_NB_Hist(2,1).String) &...
        MIAData.NB.Num{i}>=str2double(h.Mia_NB_Hist(1,2).String) &...
        MIAData.NB.Num{i}<=str2double(h.Mia_NB_Hist(2,2).String) &...
        MIAData.NB.Eps{i}>=str2double(h.Mia_NB_Hist(1,3).String) &...
        MIAData.NB.Eps{i}<=str2double(h.Mia_NB_Hist(2,3).String);
    switch h.Mia_NB_1DHist.Value
        case 1
            h.Plots.NB(4).YData=MIAData.NB.PCH{i};
            h.Plots.NB(4).XData=0:numel(MIAData.NB.PCH{i});
            h.Mia_NB_Axes(4).XLabel.String='Counts per pixel';
        case 2
            ...
            h.Plots.NB(4).YData=histc(MIAData.NB.Int{i}(Use),h.Plots.NB(4).XData);
        ...
        case 4
            ...
            h.Plots.NB(4).YData=histc(MIAData.NB.Eps{i}(Use),h.Plots.NB(4).XData);
            h.Mia_NB_Axes(4).XLabel.String='Brightness [kHz]';
    end
    h.Mia_NB_Axes(4).XLim=[h.Plots.NB(4).XData(1)-diff(h.Plots.NB(4).XData(1:2)),...
                           h.Plots.NB(4).XData(end)+diff(h.Plots.NB(4).XData(1:2))];
end
...
for i=Auto
    MaxPhotons=ceil(max(max(max(MIAData.Data{floor(i*1.5),2}))));
    MIAData.NB.PCH{floor(i*1.5)}=histc(double(MIAData.Data{i,2}(:)),0:MaxPhotons);
    MIAData.NB.Int{floor(i*1.5)}=mean(double(MIAData.Data{i,2}),3);
    MIAData.NB.Std{floor(i*1.5)}=std(double(MIAData.Data{i,2}),0,3);
    MIAData.NB.Num{floor(i*1.5)}=MIAData.NB.Int{i}.^2./(MIAData.NB.Std{i}.^2-MIAData.NB.Int{i});
    MIAData.NB.Eps{floor(i*1.5)}=(MIAData.NB.Std{i}.^2-MIAData.NB.Int{i})./MIAData.NB.Int{i};
    
    MIAData.NB.Int{floor(i*1.5)}=MIAData.NB.Int{floor(i*1.5)}/str2double(h.Mia_NB_Pixel.String)*10^3;
    MIAData.NB.Std{floor(i*1.5)}=MIAData.NB.Std{floor(i*1.5)}/str2double(h.Mia_NB_Pixel.String)*10^3;
    MIAData.NB.Eps{floor(i*1.5)}=MIAData.NB.Eps{floor(i*1.5)}/str2double(h.Mia_NB_Pixel.String)*10^3;
end

if Cross
    MaxPhotons=ceil(max(max(max(MIAData.Data{1,2}+MIAData.Data{2,2}))));
    MIAData.NB.PCH{2}=histc(double(MIAData.Data{1,2}(:)+MIAData.Data{2,2}(:)),0:MaxPhotons);
    MIAData.NB.Int{2}=sqrt(double(MIAData.NB.Int{1}).*double(MIAData.NB.Int{3}));
    MIAData.NB.Std{2}=sqrt(sum(double(MIAData.Data{1,2})-MIAData.NB.Int{1},3).*sum(double(MIAData.Data{2,2})-MIAData.NB.Int{2},3));
    MIAData.NB.Num{2}=MIAData.NB.Int{2}.^2./(MIAData.NB.Std{2}.^2-MIAData.NB.Int{2});
    MIAData.NB.Eps{2}=(MIAData.NB.Std{2}.^2-MIAData.NB.Int{2})./MIAData.NB.Int{2}; 
    
    MIAData.NB.Int{2}=MIAData.NB.Int{floor(i*1.5)}/str2double(h.Mia_NB_Pixel.String)*10^3;
    MIAData.NB.Std{2}=MIAData.NB.Std{floor(i*1.5)}/str2double(h.Mia_NB_Pixel.String)*10^3;
    MIAData.NB.Eps{2}=MIAData.NB.Eps{floor(i*1.5)}/str2double(h.Mia_NB_Pixel.String)*10^3;
end

i=channel(1);
h.Mia_NB_Hist(1,1).String=num2str(max([0, mean2(MIAData.NB.Int{i})-3*mean2(MIAData.NB.Std{i})]));
h.Mia_NB_Hist(2,1).String=num2str(min([max(max(max(MIAData.NB.Int{i}))), mean2(MIAData.NB.Int{i})+3*mean2(MIAData.NB.Std{i})]));
h.Mia_NB_Hist(3,1).String='50';

h.Mia_NB_Hist(1,2).String='0';
NoOutliers=sort(MIAData.NB.Num{i}(:));
NoOutliers=NoOutliers(1:round(0.9*numel(NoOutliers)));
NoOutliers=NoOutliers(NoOutliers>0);
h.Mia_NB_Hist(2,2).String=num2str(mean(NoOutliers)+3*std(NoOutliers));
h.Mia_NB_Hist(3,2).String='50';

h.Mia_NB_Hist(1,3).String='0';
NoOutliers=sort(MIAData.NB.Eps{i}(:));
NoOutliers=NoOutliers(1:round(0.9*numel(NoOutliers)));
NoOutliers=NoOutliers(NoOutliers>0);
h.Mia_NB_Hist(2,3).String=num2str(mean(NoOutliers)+3*std(NoOutliers));
