...
#include "sys/tcpda_cuda.h"                                /* Data Structure of Calculations */
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    size_t size_burst_matrix, size_p_matrix;
	...
	/* tcpda struct */
    check_cuda_errors( cudaMalloc((void **) &d_thread_data, sizeof(tcpda_data)) );

	/* bursts */
    check_cuda_errors( cudaMalloc((void **) &d_fbb, size_burst_matrix) );   
    check_cuda_errors( cudaMalloc((void **) &d_fbg, size_burst_matrix) );   
    check_cuda_errors( cudaMalloc((void **) &d_fbr, size_burst_matrix) );   
    check_cuda_errors( cudaMalloc((void **) &d_fgg, size_burst_matrix) );   
    ...
    check_cuda_errors( cudaMalloc((void **) &d_BGbg, size_burst_matrix) );   
    check_cuda_errors( cudaMalloc((void **) &d_BGbr, size_burst_matrix) );   
    check_cuda_errors( cudaMalloc((void **) &d_BGgg, size_burst_matrix) );   
    check_cuda_errors( cudaMalloc((void **) &d_BGgr, size_burst_matrix )); 

	/* P Matrix */
    check_cuda_errors( cudaMalloc((void **) &d_p_bb, size_p_matrix) );   
    check_cuda_errors( cudaMalloc((void **) &d_p_bg, size_p_matrix) );   
    check_cuda_errors( cudaMalloc((void **) &d_p_gr, size_p_matrix) );   

    check_cuda_errors( cudaMalloc((void **) &d_out_matrix_device,(burst_rows * p_rows * sizeof(float))) );   
    check_cuda_errors( cudaMalloc((void **) &d_P_binomial, (burst_rows * p_rows * sizeof(float))) );   
    check_cuda_errors( cudaMalloc((void **) &d_P_trinomial, (burst_rows * p_rows * sizeof(float))) );   
    check_cuda_errors( cudaMalloc((void **) &d_burst_cols, sizeof(int)) );   
    check_cuda_errors( cudaMalloc((void **) &d_burst_rows, sizeof(int)) ); 
    check_cuda_errors( cudaMalloc((void **) &d_p_cols, sizeof(int)) );   
    check_cuda_errors( cudaMalloc((void **) &d_p_rows, sizeof(int)) ); 

    check_cuda_errors( cudaMalloc((void **) &d_thread_count, sizeof(int)) );   
    check_cuda_errors( cudaMalloc((void **) &d_block_count, sizeof(int)) ); 


    /* Copy the content from the host to the device (gpu) and set calculation memory to zero. */
    check_cuda_errors( cudaMemset(d_out_matrix_device, 0, (burst_rows * p_rows * sizeof(float))) );
    check_cuda_errors( cudaMemset(d_P_binomial, 0, (burst_rows * p_rows * sizeof(float))) );
    check_cuda_errors( cudaMemset(d_P_trinomial, 0, (burst_rows * p_rows * sizeof(float))) );

    check_cuda_errors( cudaMemcpy(d_fbb, ((float *)mxGetData(prhs[0])), size_burst_matrix, cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(d_fbg, ((float *)mxGetData(prhs[1])), size_burst_matrix, cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(d_fbr, ((float *)mxGetData(prhs[2])), size_burst_matrix, cudaMemcpyHostToDevice) );
    ...
    check_cuda_errors( cudaMemcpy(d_BGbr, ((float *)mxGetData(prhs[12])), size_burst_matrix, cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(d_BGgg, ((float *)mxGetData(prhs[13])), size_burst_matrix, cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(d_BGgr, ((float *)mxGetData(prhs[14])), size_burst_matrix, cudaMemcpyHostToDevice) );
    
    check_cuda_errors( cudaMemcpy(d_p_bb, ((float *)mxGetData(prhs[15])), size_p_matrix, cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(d_p_bg, ((float *)mxGetData(prhs[16])), size_p_matrix, cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(d_p_gr, ((float *)mxGetData(prhs[17])), size_p_matrix, cudaMemcpyHostToDevice) );

    check_cuda_errors( cudaMemcpy(d_burst_cols, ((int *)&burst_cols), sizeof(int), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(d_burst_rows, ((int *)&burst_rows), sizeof(int), cudaMemcpyHostToDevice) );

    check_cuda_errors( cudaMemcpy(d_p_cols, ((int *)&p_cols), sizeof(int), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(d_p_rows, ((int *)&p_rows), sizeof(int), cudaMemcpyHostToDevice) );

    /* Update the device pointer to the real data. */
    check_cuda_errors( cudaMemcpy(&(d_thread_data->fbb), &d_fbb, sizeof(float*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->fbg), &d_fbg, sizeof(float*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->fbr), &d_fbr, sizeof(float*), cudaMemcpyHostToDevice) );
    ...
    check_cuda_errors( cudaMemcpy(&(d_thread_data->BGbr), &d_BGbr, sizeof(float*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->BGgg), &d_BGgg, sizeof(float*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->BGgr), &d_BGgr, sizeof(float*), cudaMemcpyHostToDevice) );

    check_cuda_errors( cudaMemcpy(&(d_thread_data->p_bb), &d_p_bb, sizeof(float*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->p_bg), &d_p_bg, sizeof(float*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->p_gr), &d_p_gr, sizeof(float*), cudaMemcpyHostToDevice) );    

    check_cuda_errors( cudaMemcpy(&(d_thread_data->burst_cols), &d_burst_cols, sizeof(int*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->burst_rows), &d_burst_rows, sizeof(int*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->p_cols), &d_p_cols, sizeof(int*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->p_rows), &d_p_rows, sizeof(int*), cudaMemcpyHostToDevice) );

    check_cuda_errors( cudaMemcpy(&(d_thread_data->out_matrix_device), &d_out_matrix_device, sizeof(float*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->P_binomial), &d_P_binomial, sizeof(float*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->P_trinomial), &d_P_trinomial, sizeof(float*), cudaMemcpyHostToDevice) );

    check_cuda_errors( cudaMemcpy(d_thread_count, ((int *)&thread_count), sizeof(int), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(d_block_count, ((int *)&block_count), sizeof(int), cudaMemcpyHostToDevice) );

    check_cuda_errors( cudaMemcpy(&(d_thread_data->thread_count), &d_thread_count, sizeof(int*), cudaMemcpyHostToDevice) );
    check_cuda_errors( cudaMemcpy(&(d_thread_data->block_count), &d_block_count, sizeof(int*), cudaMemcpyHostToDevice) );
    ...
    check_cuda_errors(cudaMemcpy((float *)mxGetData(plhs[0]), d_out_matrix_device, (p_rows * burst_rows * sizeof(float)), cudaMemcpyDeviceToHost) );  
