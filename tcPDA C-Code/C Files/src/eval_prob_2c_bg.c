/* eval_prob_2c_bg.c - Evaluate probabilty for 2-Color PDA after 
 *       green excitation
 *
 *========================================================*/
...
#ifdef _WIN32
static unsigned __stdcall eval_prob_2c_bg(void *data);      // stdcall necessary for MSVC
#else
void *eval_prob_2c_bg(void *data);                          // cdecl for POSIX (pthreads)
#endif
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    if(nrhs!=7) {
        mexErrMsgIdAndTxt("PDA:eval_prob_2c_bg:nrhs","18 inputs required.");
    }
    if(nlhs!=1) {
        mexErrMsgIdAndTxt("PDA:eval_prob_2c_bg:nlhs","One output required.");
    }
    ...
    p_cols = mxGetN(prhs[6]);
    p_rows = mxGetM(prhs[6]);
    ...
    /* Bursts  */
    thread_data->fgg = mxGetPr(prhs[0]); /*Sets "fgg" to be the first of the input values*/
    thread_data->fgr = mxGetPr(prhs[1]);
    
    /* Backgrounds  */
    thread_data->NBGgg = mxGetScalar(prhs[2]); /*Sets "NBGbb" to be the sixth of the input values*/
    thread_data->NBGgr = mxGetScalar(prhs[3]);    

    thread_data->BGgg = mxGetPr(prhs[4]);
    thread_data->BGgr = mxGetPr(prhs[5]);
   
    /* Probabilities */
    thread_data->p_gr = mxGetPr(prhs[6]);
    ...
    thread_data->p_cols = mxGetN(prhs[6]);
    thread_data->p_rows = mxGetM(prhs[6]);
    ...
    for (int i=0; i < num_cores; i++) {
        ...
        thread_list[i] = (HANDLE)_beginthreadex(
            ...
            0,                        /* Thread stack size */
            &eval_prob_2c_bg,         /* Thread starting address - address of function*/
    ...
    for (int i=0; i < num_cores; i++) {
        ...
        if (pthread_create(
            ...
            eval_prob_2c_bg,                   /* Pointer to the function call (entry point) */
            (void *) &thread_data_list[i]))     /* Thread arguments, our tcpda data struct. */  
        {
            fprintf(stderr, "Error creating thread\n");
            abort();
...
#endif
{
   ...
   for (size_t i=0; i < thread_data->p_rows; i++) {
        for (size_t j=thread_data->thread_id; j<thread_data->burst_rows; j=j+thread_data->num_cores) {
            thread_data->out_matrix[i][j] = P_binomial[i][j] ;
        }
    }
    /* Clean up */
