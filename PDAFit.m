...
if isempty(h.GlobalPDAFit)
    ...
    h.Menu.EstimateErrorMCMC = uimenu(...
        ...
        'Callback',@Start_PDA_Fit);
    h.Menu.EstimateErrorSupportPlaneAnalysis = uimenu(...
        'Parent',h.Menu.EstimateError,...
        'Label','Estimate Error from Support Plane Analysis',...
        'Tag','EstimateErrorSPA',...
        'Callback',@Start_PDA_Fit);
    %%% Info Menu
    %     h.Menu.Info = uimenu(...
    %         'Parent',h.GlobalPDAFit,...
    ...
    h.SettingsTab.XAxisUnit_Menu = uicontrol(...
        ...
        'Value',1,...
        'Units','normalized',...
        'String',{'Proximity Ratio','FRET efficiency','log(SD/SA)','Distance'},...
        'TooltipString','<html>Choose the quantity to be used for the x-axis.<br>Proximity Ratio: Apparent FRET efficiency from uncorrected photon counts.<br>log(SD/SA): Decadic logarithm of the ratio of the donor and acceptor (FRET) photons from uncorrected photon counts.<br>FRET efficiency: Corrected FRET efficiency.<br>Distance: Distance in Angstrom calculated from the corrected FRET efficiency.</html>',...
        'FontSize',12,...
        'Callback',{@Update_Plots,3,1},...
    ...
    h.SettingsTab.OuterBins_Fix = uicontrol(...
        ...
        'Tooltipstring', 'Ignore outer proximity ratio histogram bins during fitting. Does not work for MLE fitting!',...
        'Callback',{@Update_Plots,3},...
        'Position',[0.8 0.3 0.2 0.15],...
        'Tag','OuterBins_Fix');
    h.SettingsTab.UseStasAlgorithm = uicontrol(...
        'Style','checkbox',...
        'Parent',h.SettingsTab.Panel,...
        'BackgroundColor', Look.Back,...
        'ForegroundColor', Look.Fore,...
        'Units','normalized',...
        'Value',UserValues.PDA.SeidelPDA,...
        'FontSize',12,...
...
%         'Position',[0.65 0.05 0.1 0.15]);
    h.SettingsTab.SampleGlobal = uicontrol(...
        ...
        'ForegroundColor', Look.Fore,...
        'Style','text',...
        'String','Blocksize for Sample-based Global:',...
        'Tooltipstring', 'Every loaded dataset needs to have the same number of TWs!',...
        'Callback', {@Update_Plots, 0},...
        'Position',[0.6 0.025 0.15 0.175]);
...
for i = 1:numel(FileName)
    ...
    if exist(fullfile(PathName{i},FileName{i}), 'file') == 2
        ...
        elseif exist('SavedData','var') % file has been saved before in GlobalPDAFit and contains PDAData (named SavedData)
            ...
            if isfield(SavedData,'HalfGlobalSubsetSize')
...
corrections.Gamma_GR = 1;
corrections.FoersterRadius = 50;
background = struct;
    ...
    case {2,3}
        ...
        if ~isempty(Active)
            ...
            switch PDAMeta.xAxisUnit
                ...
                case 'log(SD/SA)'
                    Prox = real(log10(PDAData.Data{i}.NG(valid)./PDAData.Data{i}.NF(valid)));
                    h.SingleTab.Main_Axes.XLabel.String = 'log(SD/SA)';
                case {'FRET efficiency','Distance'}
                    NF_cor = PDAData.Data{i}.NF(valid) - PDAData.timebin(i)*PDAMeta.BGacc(i);
                    ND_cor = PDAData.Data{i}.NG(valid) - PDAData.timebin(i)*PDAMeta.BGdonor(i);
                    % Schuler-type correction of photon counts for direct excitation
...
UserValues.PDA.SeidelPDA = h.SettingsTab.UseStasAlgorithm.Value;
...
if (any(PDAMeta.PreparationDone(PDAMeta.Active) == 0)) || ~isfield(PDAMeta,'eps_grid')
    ...
    for i  = find(PDAMeta.Active)'
        ...
        %%% generate histogram library
        %%% Calculate grid of probabilites for acceptor photon emission
        ...
        PDAMeta.PN{i} = PN;
        PDAMeta.maxN{i} = maxN;
        %%% Precaclculations related to MLE and lifetimePDA
        if calc && any(strcmp(h.SettingsTab.PDAMethod_Popupmenu.String{h.SettingsTab.PDAMethod_Popupmenu.Value},{'MLE'}))
...
PDAMeta.SampleGlobal = cell2mat(h.FitTab.Table.Data(end-1,4:3:end-1));
...
if ~do_global
    ...
    for i = find(PDAMeta.Active)'
        ...
        switch obj
            ...
            case {h.Menu.EstimateErrorHessian,h.Menu.EstimateErrorMCMC,h.Menu.EstimateErrorSupportPlaneAnalysis}
                ...
                switch obj
                    ...
                    case h.Menu.EstimateErrorSupportPlaneAnalysis
                        %%% Support Plane Analysis according to the
                        %%% description in the Lakowicz
                        %%%
                        ...
                        %%% store previous fitparams and fixed status
                        fp = PDAMeta.FitParams(i,:);
                        ff = PDAMeta.Fixed(i,:);
                        %%% Read out best chi2
                        ...
                        for p = 1:numel(params)
                            ...
                            A_p = A; A_p(params(p),params(p)) = 1;
                            fixed_p = fixed; fixed_p(params(p)) = 1;
                            fitpar_p = fitpar;
                            PDAMeta.Fixed(i,:) = fixed_p;
                            %%% loop over range of values and
                            for r = 1:numel(range)
    ...
    % Read sample-based global parameters from fit table
    %PDAMeta.SampleGlobal = false(1,numel(PDAMeta.Global));
    if any(PDAMeta.SampleGlobal)
...
%         end
    ...
    switch obj
        ...
        case {h.Menu.EstimateErrorHessian,h.Menu.EstimateErrorMCMC,h.Menu.EstimateErrorSupportPlaneAnalysis}
            ...
            switch obj
                case h.Menu.EstimateErrorMCMC %%% additionally, refine by doing mcmc sampling
                ...
                case h.Menu.EstimateErrorSupportPlaneAnalysis
                    ...
                    global0 = PDAMeta.Global;
                    fitparams0 = PDAMeta.FitParams;
                    for p = 1:numel(params)
                        %%% fix parameter at value
                        ...
                        fitpar_p = fitparams0(1,PDAMeta.Global);
                        LB_p = PDAMeta.LB(PDAMeta.Global);
                        UB_p = PDAMeta.UB(PDAMeta.Global);
                        for i=find(PDAMeta.Active)'
                            %%% Concatenates initial values and bounds for non fixed parameters
                            fitpar_p = [fitpar_p fitparams0(i, ~PDAMeta.Fixed(i,:)& ~PDAMeta.Global)];
                            LB_p=[LB_p PDAMeta.LB(~PDAMeta.Fixed(i,:) & ~PDAMeta.Global)];
                            UB_p=[UB_p PDAMeta.UB(~PDAMeta.Fixed(i,:) & ~PDAMeta.Global)];
                        end
...
function fitpar = optimize_all_methods_SPA(h,fitfun,fitpar,LB,UB,fixed,A,b)
global PDAMeta
%%% wrapper function for calling one function with all optimization
%%% routines embedded
switch h.SettingsTab.FitMethod_Popupmenu.String{h.SettingsTab.FitMethod_Popupmenu.Value}
    case 'Simplex'
    ...
    case 'Gradient-based (lsqnonlin)'
        PDAMeta.FitInProgress = 2; % indicate that we want a vector of residuals, instead of chi2, and that we only pass non-fixed parameters
        fitopts = optimoptions('lsqnonlin','MaxFunEvals', 1E4,'Display','iter','OptimalityTolerance',1E-3);
        fitpar(~fixed) = lsqnonlin(fitfun,fitpar(~fixed),LB(~fixed),UB(~fixed),fitopts);
    case 'Gradient-based (fmincon)'
        fitopts = optimoptions('fmincon','MaxFunEvals',1E4,'Display','iter');%,'PlotFcns',@optimplotfvalPDA);
        fitpar = fmincon(fitfun, fitpar,[],[],A,b,LB,UB,[],fitopts);
...
function fitpar = optimize_all_methods_SPA_global(h,fitfun,fitpar,LB,UB)
global PDAMeta
%%% wrapper function for calling one function with all optimization
%%% routines embedded
%% Do Fit
switch h.SettingsTab.FitMethod_Popupmenu.String{h.SettingsTab.FitMethod_Popupmenu.Value}
    case 'Simplex'
        fitopts = optimset('MaxFunEvals', 1E4,'Display','iter','MaxIter',100,'TolFun',1E-2,'TolX',1E-3,'PlotFcns',@optimplotfval);
        fitpar = fminsearchbnd(fitfun, fitpar, LB, UB, fitopts);
    case 'Gradient-based (lsqnonlin)'
        PDAMeta.FitInProgress = 2; % indicate that we want a vector of residuals, instead of chi2, and that we only pass non-fixed parameters
        fitopts = optimoptions('lsqnonlin','MaxFunEvals', 1E4,'Display','iter','OptimalityTolerance',1E-3,'StepTolerance',1E-5);
        fitpar = lsqnonlin(fitfun,fitpar,LB,UB,fitopts);
    case 'Gradient-based (fmincon)'
        fitopts = optimoptions('fmincon','MaxFunEvals',1E4,'Display','iter');%,'PlotFcns',@optimplotfvalPDA);
        fitpar = fmincon(fitfun, fitpar,[],[],[],[],LB,UB,[],fitopts);
...
switch mode
    case 0 %%% Updates whole table (Open UI)
        ...
        % fill in fields for sample-based global
        Data(end-1,3:3:end) = deal({'S'});
        Data(end-1,4:3:end) = deal({false});
            ...
            case h.FitTab.Table
                ...
                elseif mod(e.Indices(2)-4,3)==0 && e.Indices(2)>=4 && e.Indices(1)<size(tab.Data,1)-1
                    ...
                    if NewData
                        ...
                        %%% global was disabled
                        %%% also disable sample-based global
                        tab.Data(end-1,e.Indices(2)) = {false};
                    end
                ...
                elseif mod(e.Indices(2)-2,3)==0 && e.Indices(2)>=2 && e.Indices(1)<size(tab.Data,1)-1
                    ...
                    if tab.Data{e.Indices(1),e.Indices(2)+2}
                        if ~tab.Data{end-1,e.Indices(2)+2} % check for sample-based global
                            ...
                            block = floor(e.Indices(1)./blocksize);
                            % synchronize value for all files of this block
                            tab.Data((blocksize*(block-1)+1):(blocksize*block),e.Indices(2))=deal({NewData});
                        end
                    else
                        %%% Not global => only changes value
                    end
                elseif e.Indices(1) == size(tab.Data,1)-1 && mod(e.Indices(2)-3,3)==0
                    %%% Field for sample-based global was changed, revert
                    %%% back to default label
                    tab.Data(e.Indices(1),e.Indices(2)) = {'S'};
                elseif e.Indices(1) == size(tab.Data,1)-1 && mod(e.Indices(2)-4,3)==0
                    %%% Sample-based global logical field was changed
                    if NewData == 1
                        %%% enable global also
                        ...
                        blocksize = str2double(h.SettingsTab.TW_edit.String);
                        % synchronize value for all files of this block
                        for block = 1:floor((size(tab.Data,1)-3)/blocksize)
                            % get the average value of block
                            val = mean(cellfun(@str2double,tab.Data((blocksize*(block-1)+1):(blocksize*block),e.Indices(2)-2)));
                            tab.Data((blocksize*(block-1)+1):(blocksize*block),e.Indices(2)-2)=deal({num2str(val)});
                        end
                    else
                        %%% disable global
                        tab.Data(1:end-2,e.Indices(2)) = deal({false});
                    end
                end               
