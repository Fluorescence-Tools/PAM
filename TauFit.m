    ...
    case {h.Fit_Button}
        ...
        if h.MCMC_Error_Estimation_Menu.Value
            ...
            if ~exist('Decay_FitRange','var')
                Decay_FitRange = Decay(ignore:end);
            end
            %%% define log-likelihood function, which is just the negative of the chi2 divided by two! (do not use reduced chi2!!!)
            loglikelihood = @(x) (-1/2)*sum((ModelFun(x,xdata)./sigma_est_fit-Decay_FitRange./sigma_est_fit).^2);
            %%% Sample
            ...
            if acceptance < 0.01
                fprintf('Acceptance was too low at %.4f!\n',acceptance);
            ...
            fprintf('Done. Performed %d steps in %.2f seconds.\nAcceptance was %.2f.\nSpacing used was %d.\n',nsamples,toc,acceptance,spacing);
