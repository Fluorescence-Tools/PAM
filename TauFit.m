...
TauFitData.ScatShift = 0;
TauFitData.ScatrelShift = 0;
TauFitData.Ignore = 1;
...
%         UserValues.PIE.From(PIEChannel_Per):min([UserValues.PIE.To(PIEChannel_Per) end]) );
    ...
    % Should data be plotted at this point?
    ...
    h.Plots.Scat_Par.XData = TauFitData.XData_Par*TACtoTime;
    h.Plots.Scat_Per.XData = TauFitData.XData_Per*TACtoTime;
    h.Plots.Decay_Par.YData = TauFitData.hMI_Par;
    h.Plots.Decay_Per.YData = TauFitData.hMI_Per;
    h.Plots.IRF_Par.YData = TauFitData.hIRF_Par;
    h.Plots.IRF_Per.YData = TauFitData.hIRF_Per;
    h.Plots.Scat_Par.YData = TauFitData.hScat_Par;
    h.Plots.Scat_Per.YData = TauFitData.hScat_Per;
    ...
    %%% Scat Length has the same limits as the Length property
    TauFitData.ScatLength = TauFitData.MaxLength;
    %%% Scat Shift has the same limits as the perp shift property
    h.ScatShift_Slider.Min = -floor(TauFitData.MaxLength/10);
    h.ScatShift_Slider.Max = floor(TauFitData.MaxLength/10);
    h.ScatShift_Slider.Value = 0;
    TauFitData.ScatShift = 0;
    h.ScatShift_Edit.String = num2str(TauFitData.ScatShift);
    
    %%% Scat rel. Shift has the same limits as the perp shift property
    h.ScatrelShift_Slider.Min = -floor(TauFitData.MaxLength/10);
    h.ScatrelShift_Slider.Max = floor(TauFitData.MaxLength/10);
    h.ScatrelShift_Slider.Value = 0;
    TauFitData.ScatrelShift = 0;
    h.ScatrelShift_Edit.String = num2str(TauFitData.ScatrelShift);
...
switch obj
    ...
    case {h.Length_Slider, h.Length_Edit}
        ...
        %%% Correct if ScatLength exceeds the Length
        if TauFitData.ScatLength > TauFitData.Length
            TauFitData.ScatLength = TauFitData.Length;
        end
    ...
    case {h.ScatShift_Slider, h.ScatShift_Edit}
        ...
        if obj == h.ScatShift_Slider
            TauFitData.ScatShift = floor(obj.Value);
        elseif obj == h.ScatShift_Edit
            TauFitData.ScatShift = str2double(obj.String);
        end
    case {h.ScatrelShift_Slider, h.ScatrelShift_Edit}
        %%% Update Value
        if obj == h.ScatrelShift_Slider
            TauFitData.ScatrelShift = floor(obj.Value);
        elseif obj == h.ScatrelShift_Edit
            TauFitData.ScatrelShift = str2double(obj.String);
        end
...
if isprop(obj,'Style')
    switch obj.Style
        ...
        case 'edit'
            ...
            h.ScatShift_Slider.Value = TauFitData.ScatShift;
            h.ScatrelShift_Slider.Value = TauFitData.ScatrelShift;
            h.FitPar_Table.Data{end,1} = TauFitData.IRFShift;
...
tmp = circshift(TauFitData.hMI_Per,[TauFitData.ShiftPer,0])';
h.Plots.Decay_Per.YData = tmp((TauFitData.StartPar+1):TauFitData.Length);
...
tmp = circshift(TauFitData.hIRF_Par,[0,TauFitData.IRFShift])';
h.Plots.IRF_Par.YData = tmp((TauFitData.StartPar+1):TauFitData.IRFLength);
...
tmp = circshift(TauFitData.hIRF_Per,[0,TauFitData.IRFShift+TauFitData.ShiftPer+TauFitData.IRFrelShift])';
h.Plots.IRF_Per.YData = tmp((TauFitData.StartPar+1):TauFitData.IRFLength);
%%% Apply the shift to the parallel Scat channel
h.Plots.Scat_Par.XData = ((TauFitData.StartPar:(TauFitData.Length-1)) - TauFitData.StartPar)*TACtoTime;
tmp = circshift(TauFitData.hScat_Par,[0,TauFitData.ScatShift])';
h.Plots.Scat_Par.YData = tmp((TauFitData.StartPar+1):TauFitData.Length);
%%% Apply the shift to the perpendicular Scat channel
h.Plots.Scat_Per.XData = ((TauFitData.StartPar:(TauFitData.Length-1)) - TauFitData.StartPar)*TACtoTime;
tmp = circshift(TauFitData.hScat_Per,[0,TauFitData.ScatShift+TauFitData.ShiftPer+TauFitData.ScatrelShift])';
h.Plots.Scat_Per.YData = tmp((TauFitData.StartPar+1):TauFitData.Length);
...
%%% Anders is this correct?
ScatterPer = circshift(TauFitData.hScat_Per,[0,TauFitData.ScatShift+TauFitData.ShiftPer+TauFitData.ScatrelShift]); %is it correct to introduce all these shifts here?
ScatterPattern = TauFitData.hScat_Par(1:TauFitData.Length) + 2*ScatterPer(1:TauFitData.Length);
ScatterPattern = ScatterPattern'./sum(ScatterPattern);
%%% (Scatter Pattern is not shifted any more during fitting) Anders what do
%%% you mean
IRFPer = circshift(TauFitData.hIRF_Per,[0,TauFitData.ShiftPer+TauFitData.IRFrelShift]);
IRFPattern = TauFitData.hIRF_Par(1:TauFitData.Length) + 2*IRFPer(1:TauFitData.Length);
