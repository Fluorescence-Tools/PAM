...
for i = 1:Number_of_Channels
    ...
    TauFitBurstData.IRFrelShift{i} = UserValues.BurstSearch.TauFit.IRFrelShift{i};
end
...
h.IRFrelShift_Slider.Min = -floor(TauFitBurstData.MaxLength{1}/10);
h.IRFrelShift_Slider.Max = floor(TauFitBurstData.MaxLength{1}/10);

h.IRFrelShift_Slider.Value = TauFitBurstData.IRFrelShift{1};
h.IRFrelShift_Edit.String = num2str(TauFitBurstData.IRFrelShift{1});
...
if obj == h.ChannelSelect_Popupmenu
    ...
    %%% IRF rel. Shift has the same limits as the perp shift property
    h.IRFrelShift_Slider.Min = -floor(TauFitBurstData.MaxLength{chan}/10);
    h.IRFrelShift_Slider.Max = floor(TauFitBurstData.MaxLength{chan}/10);
    h.IRFrelShift_Slider.Value = TauFitBurstData.IRFrelShift{chan};
    h.IRFrelShift_Edit.String = num2str(TauFitBurstData.IRFrelShift{chan});
end
...
switch obj
    ...
    case {h.IRFrelShift_Slider, h.IRFrelShift_Edit}
        ...
        if obj == h.IRFrelShift_Slider
            TauFitBurstData.IRFrelShift{chan} = floor(obj.Value);
        elseif obj == h.IRFrelShift_Edit
            TauFitBurstData.IRFrelShift{chan} = str2double(obj.String);
        end
end
...
for i = 1:numel(TauFitBurstData.StartPar)
    ...
    UserValues.BurstSearch.TauFit.IRFrelShift{i} = TauFitBurstData.IRFrelShift{i};
end
LSUserValues(1);
%%% Update Plot
