...
h.Export_Table = uitable(...
    ...
    'ColumnWidth',{105},...
    'Data', {h.Export_Types{6},h.Export_Types{10},h.Export_Types{13}},...
    'ColumnFormat', {h.Export_Types,h.Export_Types,h.Export_Types},...                      
    'Position',[0.6 0.01 0.395 0.7]);
...
if isempty(Phasor) && isempty(FCSFit) && isempty(Pam) && isempty(PCF) && isempty(Mia) && isempty(Sim) && isempty(TauFit) && isempty(BurstBrowser)
...
switch mode
    case 1 %%% Export plots to figure
        ...
        H.Fig = figure(...
            ...
            'Position',[50 150 NoP(2)*(Size+8*FontSize+40) NoP(1)*(Size+4*FontSize+5)]);
        whitebg([1 1 1]); 
        ...
        for i=1:NoP(2)
            for j=1:NoP(1)                
                ...
                %% Determins plot type
                for Type = 1:14
                    if strcmp(h.Export_Types{Type}, h.Export_Table.Data{j,i})
                        break;
                    end
                end
                %% Plots data
                switch Type               
                    ...
                    case {2 3} %%% On-Axes X Plots
                        ...
                        H.Res{i,j} =axes(...
                            ...
                            'NextPlot','add',...
                            'XTick',[],...
                            'Position',[10+3.2*FontSize+(i-1)*(Size+7.5*FontSize+25), 3.5*FontSize+(j-1)*(Size+5*FontSize+15)+0.8*Size, Size+4*FontSize+15, 0.2*Size]);
    ...
    case 3 %%% Change number of plots
       if Obj == h.Export_NumX
           ...
           if size(h.Export_Table.Data,2)>NumX
               h.Export_Table.Data = h.Export_Table.Data(:,1:NumX);
           elseif size(h.Export_Table.Data,2)<NumX
               h.Export_Table.Data{end,NumX} = []; 
               h.Export_Table.ColumnFormat = repmat({h.Export_Types},[1 NumX]);  
           end
