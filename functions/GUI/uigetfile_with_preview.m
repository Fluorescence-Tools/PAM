function [filename,path,type] = uigetfile_with_preview(filterSpec, prompt, folder, callback, multiSelect)
...
% $Revision: 1.0 $  $Date: 2016/11/02 23:43:54 $
  ...
  hFig = dialog('Name',prompt, 'units','pixel', 'pos',[200,200,800,600]);
  ...
  [hjFileChooser, hContainer] = javacomponent(javaComponentName, [0,0,800,600], hFig); %#ok<ASGLU>
  ...
  if ishghandle(hFig)
      ...
      path = fileparts(filename{1});
      for i = 1:numel(filename)
          [path,fn,ext] = fileparts(filename{i});
          filename{i} = [fn ext];
      end
      type = char(getappdata(hFig,'selectedType'));
...
function ActionPerformedCallback(hjFileChooser, eventData, hFig)
  switch char(eventData.getActionCommand)
      ...
      case 'ApproveSelection'
          ...
          type = hjFileChooser.getFileFilter.getDescription;
          ...
          setappdata(hFig,'selectedType', type);
          uiresume(hFig);
      otherwise
          % should never happen
  end
