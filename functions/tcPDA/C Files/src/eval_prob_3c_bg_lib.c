...
#include "sys/tcpda_lib.h"                              /* Data Structure of Calculations */
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    /* Libraries of binomial and trinomial coefficients */
    thread_data->lib_b = mxGetPr(prhs[18]);
    thread_data->lib_t = mxGetPr(prhs[19]);
                ...
                for (size_t b=0; b <= MIN(thread_data->fbg[j], thread_data->NBGbg) ; b++) {
                    ...
                    size_t bg_br;
                        ...
                        P_trinomial[i][j] = P_trinomial[i][j] +  // cumulative sum
                                ...
                                log(thread_data->p_bb[i])*(thread_data->fbb[j] - bg_bb) +
                                log(thread_data->p_bg[i])*(thread_data->fbg[j] - bg_bg) +
                                log(p_br)*(thread_data->fbr[j] - bg_br)
                                );
                ...
                for (size_t b=0; b <= MIN(thread_data->fgr[j], thread_data->NBGgr) ; b++) {
                   ...
                   P_binomial[i][j] = P_binomial[i][j] +             // cumulative sum
                           ...
                           exp(
                            ...
                            log(thread_data->p_gr[i])*(thread_data->fgr[j] - bg_gr) +
                            log((1-thread_data->p_gr[i]))*(thread_data->fgg[j] - bg_gg)
                            );
