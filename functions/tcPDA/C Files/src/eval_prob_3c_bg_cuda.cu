                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    // size_t sizeBGbb, sizeBGbg, sizeBGbr, sizeBGgg, sizeBGgr; /* Dimensions of Background Arrays */
	...
	/* Background */
    cudaMalloc((void **) &d_BGbb, (size_t)((mxGetScalar(prhs[5])+1)*sizeof(float)) );   
    cudaMalloc((void **) &d_BGbg, (size_t)((mxGetScalar(prhs[6])+1)*sizeof(float)) );   
    cudaMalloc((void **) &d_BGbr, (size_t)((mxGetScalar(prhs[7])+1)*sizeof(float)) );   
    cudaMalloc((void **) &d_BGgg, (size_t)((mxGetScalar(prhs[8])+1)*sizeof(float)) );   
    cudaMalloc((void **) &d_BGgr, (size_t)((mxGetScalar(prhs[9])+1)*sizeof(float)) ); 
    ...
    cudaMemcpy(d_BGbb, ((float *)mxGetData(prhs[10])), (size_t)((mxGetScalar(prhs[5])+1)*sizeof(float)) , cudaMemcpyHostToDevice);
    cudaMemcpy(d_BGbg, ((float *)mxGetData(prhs[11])), (size_t)((mxGetScalar(prhs[6])+1)*sizeof(float)), cudaMemcpyHostToDevice);
    cudaMemcpy(d_BGbr, ((float *)mxGetData(prhs[12])), (size_t)((mxGetScalar(prhs[7])+1)*sizeof(float)), cudaMemcpyHostToDevice);
    cudaMemcpy(d_BGgg, ((float *)mxGetData(prhs[13])), (size_t)((mxGetScalar(prhs[8])+1)*sizeof(float)), cudaMemcpyHostToDevice);
    cudaMemcpy(d_BGgr, ((float *)mxGetData(prhs[14])), (size_t)((mxGetScalar(prhs[9])+1)*sizeof(float)), cudaMemcpyHostToDevice);
...
__global__ void eval_prob_3c_bg(tcpda_data *thread_data)
{
    ...
    for (size_t i=0; i < *thread_data->p_rows; i++) {
        ...
        float p_bb = thread_data->p_bb[i];
        float p_bg =  thread_data->p_bg[i];
        float p_br = 1 - p_bb - p_bg;
        float p_gr = thread_data->p_gr[i];
        
        float prob[3] = {p_bb, p_bg, p_br};

        float* BGbb = thread_data->BGbb;
        float* BGbg = thread_data->BGbg;
        float* BGbr = thread_data->BGbr;
        float* BGgg = thread_data->BGgg;
        float* BGgr = thread_data->BGgr;
        ...
        for (size_t j=index; j < *thread_data->burst_rows; j=j+((*thread_data->thread_count) * (*thread_data->block_count))) {
            /* Read out the photon counts once here to avoid repeated memory calls */
            unsigned int fbb = (unsigned int) thread_data->fbb[j];
            unsigned int fbg = (unsigned int) thread_data->fbg[j];
            unsigned int fbr = (unsigned int) thread_data->fbr[j];
            unsigned int fgg = (unsigned int) thread_data->fgg[j];
            unsigned int fgr = (unsigned int) thread_data->fgr[j];
            int NBGbb = *thread_data->NBGbb;
            int NBGbg = *thread_data->NBGbg;
            int NBGbr = *thread_data->NBGbr;
            int NBGgg = *thread_data->NBGgg;
            int NBGgr = *thread_data->NBGgr;
            float P_binomial = 0;
            float P_trinomial = 0;
            
            /* trinomal calculation */
            int bg_bb;
            for (size_t a=0; a <= MIN(fbb, NBGbb) ; a++) {
                bg_bb = a;
                int bg_bg;
                for (size_t b=0; b <= MIN(fbg, NBGbg) ; b++) {
                    bg_bg = b;
                    int bg_br;
                    for (size_t c=0; c <= MIN(fbr, NBGbr) ; c++) {
                        bg_br = c;
                        const unsigned int bursts[3] = {
                            (unsigned int) fbb - bg_bb,
                            (unsigned int) fbg - bg_bg,
                            (unsigned int) fbr - bg_br};
            ...
            int bg_gg;
            for (size_t a=0; a <= MIN(fgg, NBGgg) ; a++) {
                bg_gg = a;
                int bg_gr;
                for (size_t b=0; b <= MIN(fgr, NBGgr) ; b++) {
                   bg_gr = b;
                   /* Subtract Background counts for FRET evaluation */
                   P_binomial += BGgg[bg_gg] *
                           BGgr[bg_gr] *
                           ran_binomial_pdf((unsigned int)(fgr - bg_gr),
                           p_gr, 
                           (unsigned int) (fgr - bg_gr +fgg - bg_gg) );
                }
            }

            /* multiply both */
            thread_data->out_matrix_device[j+(*thread_data->burst_rows * i)] += P_trinomial*P_binomial;
        }
    }
