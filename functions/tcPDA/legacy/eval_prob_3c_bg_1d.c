       ...
       
{
    ...
    for (i=0; i<rows; i++) {
        ...
        size_t bg_bb;
        for (size_t a=0; a <= GSL_MIN(fbb[i], NBGbb) ; a++) {
            bg_bb = a;
            size_t bg_bg;
            for (size_t b=0; b <= GSL_MIN(fbg[i], NBGbg) ; b++) {
                bg_bg = b;
                size_t bg_br;
                    for (size_t c=0; c <= GSL_MIN(fbr[i], NBGbr) ; c++) {
                    bg_br = c;
                    ...
                    P_trinomial[i] = P_trinomial[i] +  // cumulative sum
                            ...
                            gsl_ran_multinomial_pdf(3,background,bursts);
                    }
            }
        }
        ...
        size_t bg_gg;
        for (size_t a=0; a <= GSL_MIN(fgg[i], NBGgg) ; a++) {
            bg_gg = a;
            size_t bg_gr;
            for (size_t b=0; b <= GSL_MIN(fgr[i], NBGgr) ; b++) {
                bg_gr = b;
                // Subtract Background counts for FRET evaluation
                
               P_binomial[i] = P_binomial[i] +  // cumulative sum
                    BGgg[bg_gg] * BGgr[bg_gr] *
                    gsl_ran_binomial_pdf((unsigned int)(fgr[i]-bg_gr) ,
