...
void Simulate_Diffusion(
        ...
        double *Macrotimes, unsigned short *Microtimes, unsigned char *Channel, __int64_t *NPhotons, unsigned char *Polarization,
        unsigned long Time,
        int Map_Type, double *Map)        
{
    ...
    for (j=0;j<4;j++){
        for (i=0;i<MI_Bins;i++)
        {
            ...
            p_par[MI_Bins*j+i] = aniso_param[4*j+3]*(1+2*r_dummy)/(aniso_param[4*j+3]*(1+2*r_dummy)+(1-r_dummy));
        }
    }
    ...
    for (i=0; i<SimTime; i++) 
    {
        ...
        if (n_states > 1) {
            if (i % DynamicStep == 0) { // Only calculate dynamic transitions at larger time intervals
                ...
                // Next two lines not needed, k_dyn supplies transition probabilities
                //for (s=1;s<n_states;s++) { TRANS[s] = TRANS[s] + TRANS[s-1]; } /// Extract cummulative Transition rates
                //for (s=0; s<n_states; s++) { TRANS[s] = TRANS[s]/TRANS[n_states-1]; }  /// Calculates cummulative Transition Probabilites

                prob = equal_dist(mt);
                for (s=0; s<n_states; s++) /// Determines transition according to rates
                { if (prob<=TRANS[s]) { state = s; break;} } // Update State
            }
        }
...
{    
    ...
    Simulate_Diffusion(
        ...
        Macrotimes, Microtimes, Channel, NPhotons, Polarization,// Output parameters
