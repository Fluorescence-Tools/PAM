...
void Simulate_Diffusion(
        ...
        double *Macrotimes, unsigned short *Microtimes, unsigned char *Channel, __int64 *NPhotons, //double *PosX, double *PosY, double *PosZ,
        unsigned long Time,
...
//     double Bl;
    ...
    // hard code the nanopore here (3D map implementation takes too much memory)
    //double d = 100; // the thickness of the layer
    ...
    for (i=0; i<SimTime; i++) 
    {
        ...
        while (Invalid_Pos)
        {
            ...
            while ((New_Pos[0] < 0.0) || (New_Pos[0] > Box[0]) || (New_Pos[1] < 0.0) || (New_Pos[1] > Box[1]) || (New_Pos[2] < 0.0) || (New_Pos[2] > Box[2]))
            {
                ...
                if (Box[2] > 0) {                     
                    // periodic on the lower side, i.e. come out on top
                    if (New_Pos[2] < 0) {
                        //printf("Border crossing on bottom side, %.2f -> %.2f\n",New_Pos[2],fmod((New_Pos[2] + Box[2]), Box[2]));
                        New_Pos[2] = fmod((New_Pos[2] + Box[2]), Box[2]) ; 
                    }
                    // reflecting on the top, i.e. put back on top half
                    else if (New_Pos[2] > Box[2]) {
                        //printf("Border crossing on top side, %.2f -> %.2f\n",New_Pos[2],2*Box[2]-New_Pos[2]);
                        New_Pos[2] = 2*Box[2]-New_Pos[2];
                    }                    
                }
            }
            switch ((int)Map_Type)
            {
                case 1: case 2: case 8: /// Free Diffusion
                    ...
                    Invalid_Pos = false;
                    break;
                case 3: case 6: /// Diffusion with restricted zones (2D)
                ...
                case 9: /// Directed diffusion through nanopore
                    ...
                    // previous code based on 3D map
//                     New_Index = (int)(floor(New_Pos[0]) + Box[0]*floor(New_Pos[1]) + Box[0]*Box[1]*floor(New_Pos[2]));
//                     if (Map[New_Index] != 0) 
//                     {
//                         //printf("Valid position at %.2f %.2f %.2f\n",New_Pos[0],New_Pos[1],New_Pos[2]);
//                         PosX[i] = New_Pos[0];
...
//                     }
//                     else {
//                         //printf("Invalid position at %i %i %i.\n",(int)(floor(New_Pos[0])),(int)(floor(New_Pos[1])),(int)(floor(New_Pos[2])));
//                         //printf("Invalid position at %.2f %.2f %.2f\n",New_Pos[0],New_Pos[1],New_Pos[2]);
//                     }
                    break;
            ...
            for (k=0; k<4; k++)
            {
                if (Active[k] && ExP[4*j+k]>0) /// Only calculates, if particle is not bleached and direct excitation enabled
                {
                    ...
                    if (Ex > 1E-4) /// Only roll if Excitation is larger than 0.01% = 1E-4
                    {
                        ...
                        binomial_distribution<__int64> binomial(1, Ex);
                        ...
                        {                            
                            ...
                            {
                                binomial_distribution<__int64> binomial(1, BlP[4*k+m]);
                            ...
                            {                                
                                ...
                                if (DetP[4*m+n] > 1E-5)
                                {
                                    binomial_distribution<__int64> binomial(1, DetP[4*m+n]);
