...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  int           N_photons;                      // number of photons, passed to the function
  double        *deltaT;                        // array of interphoton times
  ...
  *******************************/
  if (nrhs != 6) {
    ...
    //scanf( "%s\n", dummy_str );
    //exit(1);
    mexErrMsgIdAndTxt("ChangePoint:InputError",
           "Wrong number of input arguments supplied. Exiting.");
    }
  else if (nrhs == 6) {
    // get the data
    deltaT = mxGetData(prhs[0]);
    N_photons = (int)mxGetNumberOfElements(prhs[0]);
    delta_t = (double)mxGetScalar(prhs[1]);
    alpha = (double)mxGetScalar(prhs[2]);
    beta = (double)mxGetScalar(prhs[3]);
    Ngmax = (int)mxGetScalar(prhs[4]);
    strcpy( in_name, mxArrayToString(prhs[5]));
    strcpy( out_name, mxArrayToString(prhs[5]));
    }
  ...
  *******************************/
  ...
  if (alpha == 0.01) {
    ...
    mexErrMsgIdAndTxt("ChangePoint:WrongErrorRate",
           "Wrong error rate supplied. Exiting.");
    }
