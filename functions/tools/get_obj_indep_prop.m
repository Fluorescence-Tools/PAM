function proplist = get_obj_indep_prop(obj)
% get_obj_indep_prop returns the list of independent properties of the object
    % ----------------------------------------------------------------------------------------------
    %
    %                                  get_obj_indep_prop
    %
    ...
    % $Revision: 11 $
    % $HeadURL: svn+ssh://biophysicsnick/home/lucp1791/core_tlbx/trunk/tools/get_obj_indep_prop.m $
    ...
    % DESCRIPTION
    % -----------
    % proplist = get_obj_indep_prop(obj) returns the list of independent properties of the given
    % object.
    % 
    % MODIFICATIONS
    % -------------
    ...
    for m = 1 : length(Props)
        ...
        if ~Props{m}.Dependent
            proplist{m} = Props{m}.Name;
        end
    end
    proplist(cellfun(@isempty, proplist)) = [];
    
end % end of function 'get_obj_indep_prop'