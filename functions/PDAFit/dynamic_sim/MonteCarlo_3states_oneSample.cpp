...
using namespace std;
// using namespace tr1;
void Simulate_States(double mBG_gg, double mBG_gr, double *R, 
        double *sigmaR, const double R0, const double cr, const double de, 
        const double ct, const double gamma, const int time_windows,
        const double SimTime, double *BSD, double *dwell_mean, 
        double *p_eq, double *k_dyn, double *PRH)
{
    ...
    std::vector<int> tw_range(time_windows);
    std::iota (std::begin(tw_range), std::end(tw_range), 0);
    ...
    for (auto i : tw_range){
        ...
        PRH[i] = ((EPSrnd0(mt)+EPSrnd1(mt)+EPSrnd2(mt)+BG_gr)
            /BSD[i]);
    }
}
...
{    
    ...
    double *k_dyn = mxGetPr(prhs[14]);
    double *PRH = (double*) mxCalloc(time_windows, sizeof(double));
    Simulate_States(mBG_gg, mBG_gr, R, sigmaR, R0, cr, de, ct, gamma, time_windows, SimTime, BSD, dwell_mean, p_eq, k_dyn, PRH);
    plhs[0] = mxCreateNumericMatrix(time_windows,1,mxDOUBLE_CLASS,mxREAL);
