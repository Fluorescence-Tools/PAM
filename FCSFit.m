...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    h.Normalize = uicontrol(...
        ...
        'ForegroundColor', Look.Fore,...
        'Style','popupmenu',...
        'String',{'None';'Fit N 3D';'Fit minus offset';'Fit G(0)';'Fit N 2D'; 'Time';'Fit N 3D minus offset';'Fit N 3D * brightness';'Time minus offset';'Fit G(0) minus offset'},...
        'Value',UserValues.FCSFit.NormalizationMethod,...
        'Callback',@Update_Plots,...
        'Position',[0.082 0.52 0.12 0.1]); 
...
if nargin > 3 %%% called from file history
    ...
    filetypes = {'*.mcor','Averaged correlation based on matlab filetype (*.mcor)';...
                  ...
                  '*.pqres','PicoQuant PQRES Files from SymphoTime (*.pqres)'};
    %%% sort according to stored filetype in UserValues
    order = 1:size(filetypes,1);
    temp = order(UserValues.FCSFit.FileType); order(temp) = [];
    order = [temp order];
    ...
    %%% 11/2019    
    if ~ismac || ~(ismac && get_macos_version()>=10)
        %%% Choose files to load
        [FileName,path,Type] = uigetfile(filetypes,...
...
switch Type
    ...
    case {8} % Kristine files from Seidel lab
        ...
        %%% change the gui
        SwitchGUI(h,'FCS');
    case {9} % PicoQuant PQRES Files from SymphoTime (*.pqres)
        FCSMeta.DataType = 'FCS averaged';
        for i=1:numel(FileName)
            if ~isempty(strfind(FileName{i},'_OFCS.pqres'))
                [CurrentData, Header] = Read_PicoQuant_PQRES(fullfile(PathName{i},FileName{i}));
            else
                fprintf('File %s is not an FCS file.',FileName{i});
                continue;
            end
            ...
            for j = 1:numel(CurrentData) % there can be either one curve or auto+cross correlation
                Data = struct;
                Data.Cor_Array = CurrentData{j}(:,2);
                Data.Cor_Times = CurrentData{j}(:,1);
                Data.Cor_Average = CurrentData{j}(:,2);
                Data.Cor_SEM = CurrentData{j}(:,3);

                Data.Counts = Header.CountRates{j};
                Data.Valid = ones(1,size(Data.Cor_Array,2));
                Data.Header = Header;
                
                %%% Updates global parameters
                FCSData.Data{end+1} = Data;
                FCSData.FileName{end+1} = Header.CurveNames{j};%FileName{i}(1:end-4);
                FCSMeta.Data{end+1,1} = FCSData.Data{end}.Cor_Times;
                FCSMeta.Data{end,2} = FCSData.Data{end}.Cor_Average;
                FCSMeta.Data{end,2}(isnan(FCSMeta.Data{end,2})) = 0;
                FCSMeta.Data{end,3} = FCSData.Data{end}.Cor_SEM;
...
for i=1:size(FCSMeta.Plots,1)
    if Active(i)
        ...
        if Normalization_Method ~= 7 && Normalization_Method ~= 3 && Normalization_Method ~= 9 && Normalization_Method ~= 10 
