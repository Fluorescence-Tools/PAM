...
%% if strcmp(method,'ensemble')
    ...
    if strcmp(GlobalTauData.Who,'External')
...
if exist('bh','var')
    if bh.SendToTauFit.equals(obj) || obj == bh.Send_to_TauFit_Button
        GlobalTauData.Who = 'BurstBrowser';
...
if any(strcmp(GlobalTauData.Who,{'Burstwise','BurstBrowser'}))
...
switch Type
    case {1} %%% Pam decay files
        ...
        for i=1:numel(FileName)
            ...
            if Type == 1
                GlobalTauData.Data{end+1} = load(fullfile(PathName{i},FileName{i}),'-mat');
                ...
                GlobalTauData.Data{end+1} = Data;
            end
            %%% Updates global parameters
            GlobalTauData.FileName{end+1} = FileName{i}(1:end-5);
            GlobalTauMeta.Data{end+1,1} = GlobalTauData.Data{end}.Cor_Times;
            GlobalTauMeta.Data{end,2} = GlobalTauData.Data{end}.Cor_Average;
            GlobalTauMeta.Data{end,2}(isnan(GlobalTauMeta.Data{end,2})) = 0;
            GlobalTauMeta.Data{end,3} = GlobalTauData.Data{end}.Cor_SEM;
            GlobalTauMeta.Data{end,3}(isnan(GlobalTauMeta.Data{end,3})) = 1;
            %%% Creates new plots
            GlobalTauMeta.Plots{end+1,1} = errorbar(...
                GlobalTauMeta.Data{end,1},...
                GlobalTauMeta.Data{end,2},...
                GlobalTauMeta.Data{end,3},...
                'Parent',h.GlobalTau_Axes);
            GlobalTauMeta.Plots{end,2} = line(...
                'Parent',h.GlobalTau_Axes,...
                'XData',GlobalTauMeta.Data{end,1},...
                'YData',zeros(numel(GlobalTauMeta.Data{end,1}),1));
            ...
            GlobalTauMeta.Plots{end,4} = line(...
                'Parent',h.GlobalTau_Axes,...
                'XData',GlobalTauMeta.Data{end,1},...
                'YData',GlobalTauMeta.Data{end,2});
            GlobalTauMeta.Params(:,end+1) = cellfun(@str2double,h.Fit_Table.Data(end-2,5:3:end-1));
        end
        %%% change the gui
...
switch mode
    case 0 %%% Called at the figure initialization
        ...
        h.Style_Table.Data=UserValues.GlobalTauFit.PlotStyleAll(1:8);        
...
global UserValues GlobalTauData FileInfo TcspcData GlobalTauMeta
h = guidata(findobj('Tag','GlobalTauFit'));

%%% check how we got here
if obj == h.Menu.OpenDecayData || strcmp(GlobalTauData.Who, 'External')
    switch obj
        case {h.Menu.OpenDecayData,h.Menu.OpenDecayDOnlyData_PQ,h.Menu.OpenIRFData_PQ,h.Menu.OpenDecayData_PQ}
            switch obj
                case h.Menu.OpenDecayData
                    ...
                    GlobalTauData.Who = 'External';
                    GlobalTauData.FileName = fullfile(PathName,FileName{1});
                    if numel(PIEchans) == 1
                ...
                case {h.Menu.OpenDecayDOnlyData_PQ,h.Menu.OpenIRFData_PQ,h.Menu.OpenDecayData_PQ}
                    ...
                    switch obj
                        case h.Menu.OpenDecayData_PQ
                            ...
                            GlobalTauData.TACRange = (1./double(TAC{1}))*1E9;
                            MI_Bins = numel(time); GlobalTauData.MI_Bins = MI_Bins;
                            TACChannelWidth = textscan(header.Meas_BinWidth,'%d ps');
                            GlobalTauData.TACChannelWidth = double(TACChannelWidth{1})*1E-3;
                            
                            if ~isfield(GlobalTauData,'External') % first time
                                GlobalTauData.External = struct;
                                GlobalTauData.External.MI_Hist = {};
                                GlobalTauData.External.IRF = {};
                                GlobalTauData.External.Scat = {};
                                GlobalTauData.External.Donly = {};
                                PIEchans = [];
                            else
                                PIEchans = h.PIEChannelPar_Popupmenu.String;
                            end
                            GlobalTauData.External.MI_Hist{end+1} = decay;
                            if numel(GlobalTauData.External.MI_Hist) == 1 % first data, there is no IRF
                                GlobalTauData.External.IRF{end+1} = zeros(size(decay));
                                GlobalTauData.External.Scat{end+1} = zeros(size(decay));
                                GlobalTauData.External.Donly{end+1} = zeros(size(decay));
                            else %%% take previous datasets IRF
                                GlobalTauData.External.IRF{end+1} = GlobalTauData.External.IRF{end};
                                GlobalTauData.External.Scat{end+1} =GlobalTauData.External.Scat{end};
                                GlobalTauData.External.Donly{end+1} = GlobalTauData.External.Donly{end}
                            end
                        ...
                        case h.Menu.OpenIRFData_PQ
                            ...
                            for i = 1:numel(GlobalTauData.External.IRF)
                                GlobalTauData.External.IRF{i} = decay;
                                GlobalTauData.External.Scat{i} = decay;
                            end
                        ...
                        case h.Menu.OpenDecayDOnlyData_PQ
                            for i = 1:numel(GlobalTauData.External.Donly)
                                GlobalTauData.External.Donly{i} = decay;
                            end
                            PIEChannel_Par = h.PIEChannelPar_Popupmenu.Value;
                            PIEChannel_Per = h.PIEChannelPer_Popupmenu.Value;
                    end                    
    ...
    chan = 4; GlobalTauData.chan = chan; 
             
    %%% Microtime Histograms
    GlobalTauData.hMI_Par{chan} = GlobalTauData.External.MI_Hist{PIEChannel_Par};
    GlobalTauData.hMI_Per{chan} = GlobalTauData.External.MI_Hist{PIEChannel_Per};

    %%% Read out the Microtime Histograms of the IRF for the two channels
    GlobalTauData.hIRF_Par{chan} = GlobalTauData.External.IRF{PIEChannel_Par}';
    GlobalTauData.hIRF_Per{chan} = GlobalTauData.External.IRF{PIEChannel_Per}';
    %%% Normalize IRF for better Visibility
    GlobalTauData.hIRF_Par{chan} = (GlobalTauData.hIRF_Par{chan}./max(GlobalTauData.hIRF_Par{chan})).*max(GlobalTauData.hMI_Par{chan});
    GlobalTauData.hIRF_Per{chan} = (GlobalTauData.hIRF_Per{chan}./max(GlobalTauData.hIRF_Per{chan})).*max(GlobalTauData.hMI_Per{chan});
    %%% Read out the Microtime Histograms of the Scatter Measurement for the two channels
    GlobalTauData.hScat_Par{chan} = GlobalTauData.External.Scat{PIEChannel_Par}';
    GlobalTauData.hScat_Per{chan} = GlobalTauData.External.Scat{PIEChannel_Per}';
    %%% Normalize Scatter for better Visibility
    if ~(sum(GlobalTauData.hScat_Par{chan})==0)
        GlobalTauData.hScat_Par{chan} = (GlobalTauData.hScat_Par{chan}./max(GlobalTauData.hScat_Par{chan})).*max(GlobalTauData.hMI_Par{chan});
    end
    if ~(sum(GlobalTauData.hScat_Per{chan})==0)
        GlobalTauData.hScat_Per{chan} = (GlobalTauData.hScat_Per{chan}./max(GlobalTauData.hScat_Per{chan})).*max(GlobalTauData.hMI_Per{chan});
    end
    %%% Generate XData
    GlobalTauData.XData_Par{chan} = 1:numel(GlobalTauData.hMI_Par{chan});%ToFromPar - ToFromPar(1);
    GlobalTauData.XData_Per{chan} = 1:numel(GlobalTauData.hMI_Per{chan});%ToFromPer - ToFromPer(1);
    ...
    % User called TauFit from Pam
    try
    GlobalTauData.TACRange = FileInfo.TACRange; % in seconds
    catch %%% This occurs when user loads Fabsurf file from Waldi
       % ask for TACrange, default is 40
       GlobalTauData.TACRange = str2double(inputdlg({'TAC Range in ns:'},'Please provide the TAC Range',1,{'40'}));
    end
    GlobalTauData.MI_Bins = FileInfo.MI_Bins;
    if ~isfield(FileInfo,'Resolution')
        % in nanoseconds/microtime bin
        GlobalTauData.TACChannelWidth = GlobalTauData.TACRange*1E9/GlobalTauData.MI_Bins;
    elseif isfield(FileInfo,'Resolution') %%% HydraHarp Data
        GlobalTauData.TACChannelWidth = FileInfo.Resolution/1000;
    end

    GlobalTauData.FileName = fullfile(FileInfo.Path, FileInfo.FileName{1}); %only the first filename is stored!
    ...
    %     TauFitData.hMI_Per{chan} = MI_Per(UserValues.PIE.From(PIEChannel_Per):min([UserValues.PIE.To(PIEChannel_Per) end]));
        ...
        GlobalTauData.hMI_Par{chan} = GlobalTauMeta.MI_Hist{detPar(1)}(...
            UserValues.PIE.From(PIEChannel_Par):min([UserValues.PIE.To(PIEChannel_Par) end]) );
        %%% Microtime Histogram of Perpendicular Channel
        GlobalTauData.hMI_Per{chan} = GlobalTauMeta.MI_Hist{detPer(1)}(...
            UserValues.PIE.From(PIEChannel_Per):min([UserValues.PIE.To(PIEChannel_Per) end]) );

        %%% Read out the Microtime Histograms of the IRF for the two channels
        GlobalTauData.hIRF_Par{chan} = UserValues.PIE.IRF{PIEChannel_Par}(UserValues.PIE.From(PIEChannel_Par):min([UserValues.PIE.To(PIEChannel_Par) end]));
        GlobalTauData.hIRF_Per{chan} = UserValues.PIE.IRF{PIEChannel_Per}(UserValues.PIE.From(PIEChannel_Per):min([UserValues.PIE.To(PIEChannel_Per) end]));
        %%% Normalize IRF for better Visibility
        GlobalTauData.hIRF_Par{chan} = (GlobalTauData.hIRF_Par{chan}./max(GlobalTauData.hIRF_Par{chan})).*max(GlobalTauData.hMI_Par{chan});
        GlobalTauData.hIRF_Per{chan} = (GlobalTauData.hIRF_Per{chan}./max(GlobalTauData.hIRF_Per{chan})).*max(GlobalTauData.hMI_Per{chan});
        %%% Read out the Microtime Histograms of the Scatter Measurement for the two channels
        GlobalTauData.hScat_Par{chan} = UserValues.PIE.ScatterPattern{PIEChannel_Par}(UserValues.PIE.From(PIEChannel_Par):min([UserValues.PIE.To(PIEChannel_Par) end]));
        GlobalTauData.hScat_Per{chan} = UserValues.PIE.ScatterPattern{PIEChannel_Per}(UserValues.PIE.From(PIEChannel_Per):min([UserValues.PIE.To(PIEChannel_Per) end]));
        %%% Normalize Scatter for better Visibility
        GlobalTauData.hScat_Par{chan} = (GlobalTauData.hScat_Par{chan}./max(GlobalTauData.hScat_Par{chan})).*max(GlobalTauData.hMI_Par{chan});
        GlobalTauData.hScat_Per{chan} = (GlobalTauData.hScat_Per{chan}./max(GlobalTauData.hScat_Per{chan})).*max(GlobalTauData.hMI_Per{chan});
        %%% Generate XData
        GlobalTauData.XData_Par{chan} = (UserValues.PIE.From(PIEChannel_Par):UserValues.PIE.To(PIEChannel_Par)) - UserValues.PIE.From(PIEChannel_Par);
        GlobalTauData.XData_Per{chan} = (UserValues.PIE.From(PIEChannel_Per):UserValues.PIE.To(PIEChannel_Per)) - UserValues.PIE.From(PIEChannel_Per);
    end 
end
%%% disable reconvolution fitting if no IRF is defined
if all(isnan(GlobalTauData.hIRF_Par{chan})) || all(isnan(GlobalTauData.hIRF_Per{chan}))
    disp('IRF undefined, disabling reconvolution fitting.');
    h.Fit_Button.Enable = 'off';
elseif all(isnan(GlobalTauData.hScat_Par{chan})) || all(isnan(GlobalTauData.hScat_Per{chan}))
    disp('Scatter pattern undefined, using IRF instead.');
    GlobalTauData.hScat_Par{chan} = GlobalTauData.hIRF_Par{chan};
    GlobalTauData.hScat_Per{chan} = GlobalTauData.hIRF_Per{chan};
else
    h.Fit_Button.Enable = 'on';
end
%%% fix wrong length of IRF or Scatter pattern
len = numel(GlobalTauData.hMI_Par{chan});
if numel(GlobalTauData.hIRF_Par{chan}) < len
    GlobalTauData.hIRF_Par{chan} = [GlobalTauData.hIRF_Par{chan},zeros(1,len-numel(GlobalTauData.hIRF_Par{chan}))];
elseif numel(GlobalTauData.hIRF_Par{chan}) > len
    GlobalTauData.hIRF_Par{chan} = GlobalTauData.hIRF_Par{chan}(1:len);
end
if numel(GlobalTauData.hIRF_Per{chan}) < len
    GlobalTauData.hIRF_Per{chan} = [GlobalTauData.hIRF_Per{chan},zeros(1,len-numel(GlobalTauData.hIRF_Per{chan}))];
elseif numel(GlobalTauData.hIRF_Per{chan}) > len
    GlobalTauData.hIRF_Per{chan} = GlobalTauData.hIRF_Per{chan}(1:len);
end
if numel(GlobalTauData.hScat_Par{chan}) < len
    GlobalTauData.hScat_Par{chan} = [GlobalTauData.hScat_Par{chan},zeros(1,len-numel(GlobalTauData.hScat_Par{chan}))];
elseif numel(GlobalTauData.hScat_Par{chan}) > len
    GlobalTauData.hScat_Par{chan} = GlobalTauData.hScat_Par{chan}(1:len);
end
if numel(GlobalTauData.hScat_Per{chan}) < len
    GlobalTauData.hScat_Per{chan} = [GlobalTauData.hScat_Per{chan},zeros(1,len-numel(GlobalTauData.hScat_Per{chan}))];
elseif numel(GlobalTauData.hScat_Per{chan}) > len
    GlobalTauData.hScat_Per{chan} = GlobalTauData.hScat_Per{chan}(1:len);
end
...
if strcmp(UserValues.TauFit.PIEChannelSelection{1},UserValues.TauFit.PIEChannelSelection{2})
    ...
    %%% set perp sliders/edit to zero
    GlobalTauData.ShiftPer{chan} = UserValues.TauFit.ShiftPer{chan};
    h.ShiftPer_Edit.String = num2str(UserValues.TauFit.ShiftPer{chan});
    h.ShiftPer_Slider.Value = UserValues.TauFit.ShiftPer{chan};
    GlobalTauData.IRFrelShift{chan} = UserValues.TauFit.IRFrelShift{chan};
    ...
    GlobalTauData.ScatrelShift{chan} = UserValues.TauFit.ScatrelShift{chan};
