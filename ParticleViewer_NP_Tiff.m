...
%%%Load either .Phr or .Tiff file
h.Load_Particle_Data= uimenu(...
    'Parent',h.ParticleViewer,...
    'Label','Load...',...
    'Tag','Load_Particle Data');
%%% Load Phr data
h.Load_Particle_Data_Phr = uimenu(...
    'Parent',h.ParticleViewer,...
    'Label','Load Particle Phasor Data',...
    'Callback',{@Load_Data, 1});
%%%% Load Tiff data
h.Load_Particle_Data_Phr = uimenu(...
    'Parent',h.ParticleViewer,...
    'Label','Load Particle Tiff Data',...
    'Callback',{@Load_Data, 3});
...
h.Particle.Menu=uicontextmenu;
h.Particle_Display_Export_Menu = uimenu(...
    'Parent', h.Particle.Menu,...
    'Label','Export to figure',...
    'Callback',{@Particle_Num, 1});
h.Particle_Display_Image.UIContextMenu = h.Particle.Menu;
h.ParticleViewer.UIContextMenu = h.Particle.Menu;
...
h.Particle_Plot3 = axes(...
    ...
    'XColor', Look.Fore,...
    'YColor', Look.Fore,...
    'Position',[0.72 0.4 0.27 0.63]);
h.Particle_Plot3.DataAspectRatio = [1 1 1];
%%%Histograms%%
h.Particle_Hist = axes(...
    'Parent',h.ParticleViewer,...
    'Units','normalized',...
    'Position',[0.74 0.1 0.24 0.28]);
h.Particle_Hist.XColor = Look.Fore;
h.Particle_Hist.YColor = Look.Fore;
...
h.Particle_Plot3_Type = uicontrol(...
    ...
    'ForegroundColor', Look.Fore,...
    'Callback',{@Particle_Num, 1},...
    'Position',[0.86 0.86 , 0.12 0.12],...
    'String',{'Position', 'Phasor', 'Lifetime vs Intensity'},...
    'Value', 1);
%%% Plot display selection
h.Particle_Hist_Type = uicontrol(...
    'Parent',h.ParticleViewer,...
    'Style','popupmenu',...
    'Units','normalized',...
    'FontSize',12,...
...
h.Text = {};
h.Particle.Menu=uicontextmenu;
h.Particle_Export_Menu = uimenu(...
    'Parent', h.Particle.Menu,...
    'Label','Export to figure',...
    'Callback',{@Particle_Num, 1});
h.Particle_Plot3.UIContextMenu = h.Particle.Menu;
h.ParticleViewer.UIContextMenu = h.Particle.Menu;

h.Particle.Menu=uicontextmenu;
h.Particle_Hist_Export_Menu1 = uimenu(...
    'Parent', h.Particle.Menu,...
    'Label','Export to figure',...
    'Callback',{@Particle_Frame, 1});
h.Particle_Hist.UIContextMenu = h.Particle.Menu;
h.ParticleViewer.UIContextMenu = h.Particle.Menu;
...
%%% Main Hist Plot Update Function%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function PlotUpdate2
% h = guidata(findobj('Tag','ParticleViewer'));
...
% end
% 
%     for i=h.Particle_FrameSlider.Value
% switch h.Particle_Hist_Type.Value
%     case 1 %%%TauP
%         YValue = ParticleViewer.TauP(:, i);
%         YValue(YValue==0) = NaN;
%         YLabel = 'Frequency';
%         XLabel = 'TauP';
%     case 2 %%% Mask overlay           
%         %%% Grayscale Image with red homogeneous particles
%         YValue = ParticleViewer.TauM(:, i);
%         YValue(isinf(YValue)) = NaN;
%         YLabel = 'Frequency';
%         XLabel = 'TauM';
%     case 3 %%% Mask gray/red               
%         %%% Grayscale Image with red scaled particles
%          TauP = ParticleViewer.TauP(:, i);
%         TauP(TauP == 0) = NaN;
%         TauM = ParticleViewer.TauM(:, i);
%         TauM(isinf(TauM)) = NaN;
%         YValue = (TauP + TauM)/2;
%         YLabel = 'Frequency';
%         XLabel = 'Lifetime (ns)';
%     case 4 %%% Mask magenta/green
%         %%% Scales Intensity image
%         YValue = ParticleViewer.Intensity(:, i);
%         YValue(YValue==0) = NaN;
%         YLabel = 'Frequency';
%         XLabel = 'Intensity (counts)';
% end
% %Update plot
% h.Particle_Hist = histogram(YValue);
% % set(axes);
...
obj2=gcbo;
if obj2==h.Particle_Hist_Export_Menu1
hfig = figure('Visible','on',...
               'Position',[100,100,1600*h.Particle_Hist.Position(4),1400*h.Particle_Hist.Position(4)]);
        ax5 = copyobj(h.Particle_Hist, hfig);
        hfig.Color = [1,1,1];
        set(ax5,'Position', get(0, 'DefaultAxesPosition'),'Color',[1 1 1], 'XColor',[0 0 0],'YColor',[0 0 0],'LineWidth',2);     
end
end
...
if ParticleViewer.TauP~=0 & ParticleViewer.TauM~=0
...
Plot_Type = h.Particle_Plot3_Type.Value;
...
obj=gcbo;
if obj==h.Particle_Export_Menu
hfig = figure('Visible','on',...
               'Position',[100,100,1600*h.Particle_Plot1.Position(2),1800*h.Particle_Plot1.Position(1)]);
        ax1 = copyobj(h.Particle_Plot1, hfig);
        ax2 = copyobj(h.Particle_Plot2, hfig);
        ax3 = copyobj(h.Particle_Plot3, hfig);
        hfig.Color = [1,1,1];
        set(ax2,'Position', [.25 .52 .35 .35],'Color',[1 1 1], 'XColor',[0 0 0],'YColor',[0 0 0],'LineWidth',2);
        set(ax3,'Position', [.62 .52 .35 .35],'Color',[1 1 1], 'XColor',[0 0 0],'YColor',[0 0 0],'LineWidth',2);
        set(ax1,'Position', get(0, 'DefaultAxesPosition'),'XColor',[0 0 0],'YColor',[0 0 0],'LineWidth',2);     
end
obj1=gcbo;
if obj1==h.Particle_Display_Export_Menu
hfig = figure('Visible','on',...
               'Position',[100,100,500*h.Particle_Display.Position(4),500*h.Particle_Display.Position(4)]);
        ax4 = copyobj(h.Particle_Display, hfig);
        hfig.Color = [1,1,1];
        set(ax4,'Position', get(0, 'DefaultAxesPosition'));     
end
end
...
%         end
    ...
    case 2 % Load Framewise Phasor Data
        ...
        case 3 % Loads ParticleDetection Results from Tiff file
        [FileName,PathName] = uigetfile({'*.mat'},'Load Results of ParticleDetection', UserValues.File.PhasorPath, 'MultiSelect', 'off');
        if ~iscell(FileName) && all(FileName==0)
            disp('No file selected');
            return
        end
