...
%                            and NUMEL(Y). This is the default.
...
%                            is equal to NUMEL(X) and NUMEL(Y).
...
switch opts.Normalization
    case 'countdensity'
        binarea = bsxfun(@times,double(diff(xedges.')),...
            double(diff(yedges)));
        n = n./binarea;
    case 'cumcount'
    ...
    case 'pdf'
        binarea = bsxfun(@times,double(diff(xedges.')),...
            double(diff(yedges)));
        n = n/size(subs,1)./binarea;
    case 'cdf'
        n = cumsum(cumsum(n/size(subs,1),1),2);
end
    ...
    for i = 1:2:inputlen
        ...
        switch name
            ...
            case 'XBinEdges'
                ...
                opts.XBinEdges = reshape(value,1,[]);
            ...
            case 'YBinEdges'
                ...
                opts.YBinEdges = reshape(value,1,[]);
                % Only set NumBins field to empty if both XBinEdges and
                % YBinEdges are set, to enable BinEdges override of one
                % dimension
                if ~isempty(opts.XBinEdges)
